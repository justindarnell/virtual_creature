namespace VirtualLife.SimCore
{
    public sealed class SimCreature
    {
        ...
        public void Tick(SimWorld world, SimConfig config, float deltaTime, SimRng rng)
        {
            ...
            var target = DetermineTarget(world, config);
            if (target != null)
            {
        ...
        private WorldObject DetermineTarget(SimWorld world, SimConfig config)
        {
            if (Drives.Hunger > config.UrgentDriveThreshold)
            {
                return world.FindNearest(WorldObjectType.Food, Position);
            }

            if (Drives.Fatigue > config.UrgentDriveThreshold)
            {
